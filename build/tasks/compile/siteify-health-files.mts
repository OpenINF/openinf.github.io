...
const siteifyFile = (file, destDir, frontmatterOverrides = {}) => {
  ...
  try {
    ...
    healthFileContents = healthFileContents.replace(/^#{1,2} (.*)\n\n/m, '');
  } catch {
  ...
  // The trailing slash matters: without it Eleventy writes an extensionless
  // file rather than `<permalink>/index.html`, which it treats as fatal.
  let frontmatter = {
    title,
    permalink: `/docs/${slug}/`,
